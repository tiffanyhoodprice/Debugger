...
var singleUser = allUsers[i];
  if ( singleUser.city === joeyProfile.city) {
    joeyProfile.friends.push(singleUser.username)
  };
  if ( singleUser.job === joeyProfile.job) {
    joeyProfile.coworkers.push(singleUser.username)
  };
  var lastname = singleUser.username.split(" ")[1];
  var joeyLastname = joeyProfile.username.split(" ")[1];
  if (lastname === joeyLastname) {
    joeyProfile.family.push(singleUser.username)
  };

for (var i=0; i < allUsers.family.length; i++) {
  for (var i=0; i<allUsers.friends.length; i++) {
    if (singleUser.family[i] === singleUser.friends[i]) {
      console.log(singleUser.family[i] + 'equals' +singleUser.friends[i]);
    }
  }
